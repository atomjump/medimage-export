    ...
    class plugin_medimage_export
    {
        ...
        public function on_upload_screen()
        {
            ...
            //General pdf export
             ...
             $ret_text .= "function PDFExport(msgId) { var userDate = new Date(); var strUserDate = userDate.toString(); var url=\"" . $root_server_url . "/plugins/medimage_export/pdf-export/export.php?uniqueFeedbackId=" . $layer_name . "\"&userDateTime=' + strUserDate + '&sender_id=" . $sender_id . "\"; alert(url); window.open(url)}</script>";
             
             //'' alert(JSON.stringify(window.frames));var pdfForm = window.frames[1].document.pdfExportForm; pdfForm.target = 'myActionWin'; window.open('','myActionWin','width=500,height=300,toolbar=0'); pdfForm.submit();
             /*  <form name='pdfExportForm' id='pdfexport' action='\"" . $root_server_url . "/plugins/medimage_export/pdf-export/export.php\"' onsubmit=''  autocomplete='off' method='POST'><input type='hidden' name='uniqueFeedbackId' value='" . $layer_name . "'><input type='hidden' name='userDateTime' value='TESTTIME'><input type='hidden' name='sender_id' value='" . $sender_id . "'></form> */ 
