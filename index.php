    ...
    class plugin_medimage_export
    {
        ...
        public function on_upload_screen()
        {
            ...
            //General pdf export
             ...
             $ret_text .= "function PDFExport(msgId) { var userDate = new Date(); var strUserDate = userDate.toString(); 
jQuery('#pdfexport').attr('target', 'myActionWin'); window.open('','myActionWin','width=500,height=300,toolbar=0'); jQuery('#pdfexport').submit(); }</script> ";
            
            //Visible buttons
            $ret_text  .= "<form name='pdfExportForm' id='pdfexport' action='' onsubmit=''  autocomplete='off' method='POST'>      				<input type='hidden' name='uniqueFeedbackId' value='" . $layer_name . "'>							<input type='hidden' name='userDateTime' value='TESTTIME'><input type='hidden' name='sender_id' value='" . $sender_id . "'></form><a class=\"comment-msg-button\" href=\"javascript:\" onclick=\"return PDFExport();\"><img width=\"48\" src='" . $root_server_url . "/plugins/medimage_export/pdf128.png'></a><a class=\"comment-msg-button\" href=\"javascript:\" onclick=\"return medimagePDFExport();\"><img width=\"48\" src='" . $root_server_url . "/plugins/medimage_export/medimage_logo.png'></a><img id='pdf-waiting' src='images/ajax-loader.gif' width='16' height='16' style='display: none;'>";
